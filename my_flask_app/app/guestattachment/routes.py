...
def create_guestattachment():
    ...
    try:
        ...
        for attachment in attachments:
            ...
            # Modify the name if it already exists in guest_attachment
            new_name = original_name
            counter = 1
            while new_name in existing_names:
                new_name = f"{original_name.split('.')[0]}_{counter}.{original_name.split('.')[-1]}"
                counter += 1
            
            # Save the image using ImageManipulator
            guest_id = guest_id  # Use the guest ID from the database
            attachment_info_list,duplicate_image_status = ImageManipulator.save_bitmap_image_list([image],new_name ,guest_id,duplicate_image_status)
            attachment['Name'] = new_name
            print("attachment['Name']",attachment['Name'])
            if attachment_info_list:  # This checks if the list is not empty
