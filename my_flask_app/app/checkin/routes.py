...
from app.models import Payment, Checkin, Guest, GuestAttachment, CheckinGuest, Log, GuestVersion, Room, Country, Emirate, DocumentType, VisitPurpose, Relationship,  PaymentType, CheckinType, CardType, Checkout, RoomChange, GuestDocumentImage
...
@checkin_bp.route('/checkintable', methods=['GET'])
def checkin():
...
def get_checkin():
    ...
    # Return check-in details in JSON format
    checkin_data = {
        "Id": current_checkin.Id,
        "CheckinUID": current_checkin.CheckinUID,
        "RoomNumber": current_checkin.RoomNumber,
        "CheckinDate": current_checkin.CheckinDate.isoformat() if current_checkin.CheckinDate else None,
        "IsActive": current_checkin.IsActive,
        "ChargeExtra": current_checkin.ChargeExtra,
        "IsFeeUpdated": current_checkin.IsFeeUpdated,
        "TDFee": current_checkin.TDFee,
        "AddedAt": current_checkin.AddedAt.isoformat() if current_checkin.AddedAt else None,
        "AddedFrom": current_checkin.AddedFrom,
        "CheckinTypeId": current_checkin.CheckinTypeId,
        "PaymentId": current_checkin.PaymentId,
        "ChildEscortCount": current_checkin.ChildEscortCount,
        "AdultEscortCount": current_checkin.AdultEscortCount,
    }
...
def create_checkin():
    ...
    try:
        ...
        for guest1 in guests1:
            errors = validate_guest(guest1)
            if errors:
                print(f" Guest {guest1.get('GuestCode', '')} has errors.")
                all_errors.append({
                    "GuestCode": guest1.get("GuestCode", ""),
                    "errors": errors
                })
            else:
                print(f" Guest {guest1.get('GuestCode', '')} has no errors.")

        # Print summary
        if all_errors:
            print(" One or more guests have validation errors.")
            return jsonify({
                "hasErrors": True,
                "errorMessages": all_errors,
                "messageType": "CheckinResponse",
            ...
            print(" All guests validated successfully (no errors).")
        ...
        for guest_data in data['Guests']:
                ...
                for attachment in attachments:
                    ...
                    if not is_duplicate_in_db:
                        ...
                        new_blob = GuestDocumentImage(
                            GuestId=guest_id,
                            DocumentUID=None,
                            AttachmentCode=attachment.get('AttachmentCode') or attachment.get('attachmenttCode') or attachment['Name'],
                            FileName=image_name,
                            FileSizeKB=file_size_kb,
            ...
            if (duplicate_image_status == False):
...
def create_backdatedcheckin():
    ...
    try:
        ...
        for guest_data in data['Guests']:
            ...
            # Validation 50014: Guest Checkin date is before Checkin date
            checkin_date1 = datetime.fromisoformat(data['CheckinDateTime'])
            if guest_data['CheckinDateTime'] == None:
                guest_checkin_date = datetime.fromisoformat(data['CheckinDateTime'])
            else:
                guest_checkin_date = datetime.fromisoformat(guest_data['CheckinDateTime'])
            # guest_checkin_date = datetime.fromisoformat(guest_data['CheckinDateTime'])
            checkout_date1 = datetime.fromisoformat(data['CheckoutDateTime'])
            if guest_data['CheckoutDateTime'] == None:
                guest_checkout_date = datetime.fromisoformat(data['CheckoutDateTime'])
            else:
...
def translate_to_arabic(english_text):
    try:
        source_language = "en"
        target_language = "ar"
        translate_url = Config.GOOGLE_TRANSLATOR_URL  # Get from config

        response = requests.get(
            f"{translate_url}?tlqt=1&langpair={source_language}|{target_language}&text={english_text}&&tl_app=1"
        )

        if response.status_code == 200:
            json_result = response.json()
            
            # Extract the first available translation from "hws" list
            translated_text = json_result[0]["hws"][0] if "hws" in json_result[0] else json_result[0]["ew"]
            
            return translated_text  #  Return only the Arabic text, not full JSON
